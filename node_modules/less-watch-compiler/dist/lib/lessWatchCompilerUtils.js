...
const defaultBannerText = 'This file was generated by less-watch-compiler. Do not edit it directly -- edit the source .less file instead; changes made here will be overwritten.';
...
function buildBannerComment(banner) {
...
const lessWatchCompilerUtilsModule = {
                    ...
                    files.forEach((entry) => {
                        ...
                        fs_1.default.stat(filePath, (statErr, st) => {
                            ...
                            if (!enoent && st) {
                                ...
                                if (options.exclude && options.exclude.test(filePath))
                                    return void finalize(null);
     ...
     */
    makeWatchHandler(mainFilePath, notify) {
        return function (f, curr, prev, fileimports) {
            if (typeof f === 'object' && prev === null && curr === null) {
                ...
                if (compileResult && notify.onCompile)
                    notify.onCompile(f, compileResult);
            }
            else {
                ...
                for (const i in fileimports) {
                    for (const k in fileimports[i]) {
                        ...
                        if (existing)
                            ...
                            importersOf.set(normalizedPath, [i]);
                    }
                }
                ...
                while (queue.length > 0) {
                    ...
                    if (compileResult && notify.onImportCompile)
                        notify.onImportCompile(importer, f, compileResult);
                    const grandImporters = importersOf.get(importer);
                    if (grandImporters)
                        queue.push(...grandImporters);
                }
                if (visited.size === 0) {
                    // Not anyone's import (transitively) -- compile the changed file itself.
                    const compileResult = lessWatchCompilerUtilsModule.compileCSS(f);
                    if (compileResult && notify.onCompile)
                        notify.onCompile(f, compileResult);
                }
            }
    ...
    compileCSS(file, test) {
        ...
        if (!test) {
            ...
            lessWatchCompilerUtilsModule
                ...
                .then(() => {
                if (cacheEnabled && cachePath && fingerprintInput)
                    cache.record(cachePath, fingerprintInput, file, outPath);
            })
                .catch((error) => {
        ...
        if (banner) {
            ...
            if (result.map) {
                ...
                if (mapPath) {
                    ...
                    css = css.replace(/\/\*# sourceMappingURL=data:application\/json;base64,[\s\S]*?\*\/\s*$/, '/*# sourceMappingURL=data:application/json;base64,' + Buffer.from(shiftedMap).toString('base64') + ' */');
                }
            }
            css = bannerText + css;
        }
        await fs_1.default.promises.mkdir(path_1.default.dirname(path_1.default.resolve(outPath)), { recursive: true });
        await fs_1.default.promises.writeFile(outPath, css, 'utf8');
        if (result.map && mapPath) {
            await fs_1.default.promises.writeFile(mapPath, result.map, 'utf8');
        }
    },
    formatLessError(error) {
        ...
        if (error.extract) {
            ...
            message +=
                '\n' +
                    error.extract
                        .map((line, index) => (line !== undefined ? firstLine + index + ' ' + line : undefined))
     ...
     */
    resolveExcludePattern(userPattern) {
        if (!userPattern)
            ...
            if (c.nlink === 0) {
                ...
                setTimeout(() => {
                    ...
                    fs_1.default.access(f, fs_1.default.constants.F_OK, (accessErr) => {
                        ...
                        if (existed && !(options.ignoreDotFiles && path_1.default.basename(f)[0] === '.') && !(options.filter && options.filter(f))) {
                    ...
                    nfiles.forEach((b) => {
                        ...
                        if (!files[file]) {
                            fs_1.default.stat(file, (err, stat) => {
                                if (err || !stat)
                                    return; // raced with a delete between readdir and stat
                                ...
                                if (!stat.isDirectory() && options.filter && options.filter(b))
                                    return;
    ...
    fileWatcher(f, files, options, filelistArr, fileimportlistObj, watchCallback) {
        ...
        for (const i in fileimportlistObj[f]) {
            ...
            const hasExtension = path_1.default.extname(importSpec).length > 1;
            const importFile = hasExtension ? importSpec : importSpec + '.less';
            const importPath = path_1.default.normalize(path_1.default.dirname(f) + path_1.default.sep + importFile);
            ...
            if (options.exclude && options.exclude.test(importPath))
                continue;
            ...
            if (importAllowedExtensions.indexOf(path_1.default.extname(importPath)) === -1)
                continue;
     ...
     */
    watchExternalImportDir(importPath, files, options, filelistArr, fileimportlistObj, watchCallback) {
        const watchFolder = lessWatchCompilerUtilsModule.config.watchFolder;
        if (!watchFolder)
            return;
        const relative = path_1.default.relative(watchFolder, importPath);
        const isExternal = relative.startsWith('..') || path_1.default.isAbsolute(relative);
        if (!isExternal)
            return;
        const importDir = path_1.default.dirname(importPath);
        if (filelistArr.indexOf(importDir) !== -1)
            return;
        let stat;
        let existingEntries;
        try {
            stat = fs_1.default.statSync(importDir);
            existingEntries = fs_1.default.readdirSync(importDir);
        }
        catch {
        ...
        for (const entry of existingEntries) {
            const entryPath = path_1.default.join(importDir, entry);
            if (files[entryPath])
                continue;
            try {
                files[entryPath] = fs_1.default.statSync(entryPath);
            }
            catch {
