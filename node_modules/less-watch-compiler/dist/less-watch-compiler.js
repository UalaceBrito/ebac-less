...
program
    ...
    .option('--cache', 'Skip recompiling a file (under --run-once) when its content and full @import closure are unchanged since the last cached run. Off by default; for CI, restore the cache file between runs.')
    .option('--cache-path <file>', "Cache file path when --cache is set. Defaults to '<cwd>/.less-watch-compiler-cache.json'.")
    .option('--exclude <pattern>', "Additional regex pattern for paths to never watch or compile, e.g. '--exclude dist'. node_modules and .git are always excluded.")
...
if (programOption.init) {
    const scaffoldPath = path_1.default.join(cwd, 'less-watch-compiler.config.json');
    if (fs_1.default.existsSync(scaffoldPath)) {
    ...
    fs_1.default.writeFileSync(scaffoldPath, JSON.stringify(scaffold, null, 2) + '\n', 'utf8');
...
function init() {
    ...
    if (programOption.minified !== undefined)
        lessWatchCompilerUtils.config.minified = programOption.minified;
    ...
    if (programOption.cache !== undefined)
        lessWatchCompilerUtils.config.cache = programOption.cache;
    if (programOption.cachePath)
        lessWatchCompilerUtils.config.cachePath = programOption.cachePath;
    if (programOption.exclude)
        lessWatchCompilerUtils.config.exclude = programOption.exclude;
    if (programOption.bannerText)
        lessWatchCompilerUtils.config.banner = programOption.bannerText;
    else if (programOption.banner !== undefined)
        lessWatchCompilerUtils.config.banner = programOption.banner;
    ...
    try {
        ...
        console.log('Invalid --exclude pattern ' + JSON.stringify(lessWatchCompilerUtils.config.exclude) + ': ' + err.message);
        process.exit(1);
    }
